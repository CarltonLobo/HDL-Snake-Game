...
def rom_12_bit(name, im, mask=False, rem_x=-1, rem_y=-1):
    ...
    # changed to(editor's preference)
    ...
    for y in range(y_max):
        for x in range(x_max):
            # write : color_data =
            case = format(y, 'b').zfill(row_width) + \
                format(x, 'b').zfill(col_width)
            f.write("\t\t" + str(row_width + col_width) + "'b" +
                    case + ": color_data = " + str(12) + "'b")
...
generate("img.bmp")     # <-- change your .bmp file name here
